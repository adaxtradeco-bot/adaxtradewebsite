/**
 * Infographic Config Panel - Shared Component
 * Author: Kiro AI
 * Created: 2025-01-XX
 * 
 * Purpose: Reusable component for infographic configuration in PropertyPanels
 * Includes: Type selection, Theme settings, Animation settings, Style settings
 */
'use client';
...
interface InfographicConfigPanelProps {
  value?: InfographicConfig;
  onChange: (config: InfographicConfig | undefined) => void;
  label?: string;
  showAdvancedSettings?: boolean;
}

export default function InfographicConfigPanel({
  value,
  onChange,
  label = 'Infographic',
  showAdvancedSettings = true,
}: InfographicConfigPanelProps) {
  ...
  const handleTypeChange = (type: string) => {
    ...
    const newConfig: InfographicConfig = {
      type,
  ...
  const handleThemeAnimationStyleChange = (updates: {
    theme?: InfographicTheme;
    animation?: InfographicAnimation;
    style?: InfographicStyle;
  }) => {
    ...
    <div className="border-t border-gray-300 dark:border-gray-600 pt-2">
      ...
      <label className="flex items-center text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">
        ...
        <InlineFieldHelper
          ...
          description="Visual data representation with advanced theming"
          note="Use JSON Editor to configure data, or Advanced Settings for theme/animation"
        />
      </label>
      ...
      {value?.type && showAdvancedSettings && (
        <div className="mt-2">
          ...
          >
            ...
            Advanced Settings (Theme, Animation, Style)
          </button>
        </div>
      )}
      ...
      {value?.type && showSettings && showAdvancedSettings && (
        <div className="mt-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <InfographicSettings
            ...
            style={value.style}
            onChange={handleThemeAnimationStyleChange}
          />
        </div>
      )}
