...
export default function AICopilotGridSection({ data, style }: AICopilotGridSectionProps) {
  return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="flex flex-col gap-3">
            {data.cards.map((card, idx) => (
              <div
                key={idx}
                className="bg-slate-900/50 border border-white/5 rounded-2xl p-6 transition-all duration-300 hover:border-indigo-500/30 hover:translate-x-1 cursor-default"
              >
                <div className="flex items-start gap-3 mb-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0 ${iconColorClasses[card.color]}`}>
                    {card.icon}
                  </div>
                  <div>
                ...
                <div className="text-xs text-slate-400 italic border-l-2 border-indigo-500/25 pl-2.5">
                  {card.example}
                </div>
              </div>
            ))}
          ...
          <div>
            {data.chatMock?.title && (
              <div className="text-xs font-medium tracking-wider uppercase text-slate-400 mb-3">
                {data.chatMock.title}
              </div>
            )}
            
            <div className="bg-slate-900 border border-white/10 rounded-2xl p-5 flex flex-col gap-2.5">
              {data.chatMock?.messages.map((msg, idx) => (
                ...
                >
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                    msg.type === 'ai' ? 'bg-indigo-500/20 text-indigo-300' : 'bg-green-500/20 text-green-400'
                  }`}>
                    {msg.avatar}
                  </div>
                  <div className={`max-w-[80%] px-3.5 py-2.5 rounded-xl text-xs leading-relaxed ${
                    msg.type === 'ai'
                      ...
                      : 'bg-green-500/12 border border-green-500/20 text-white rounded-tr-sm'
                  }`}>
                    {msg.isTyping ? (
            ...
            {data.mediaPlaceholder && (
              <div className="mt-4 bg-indigo-500/5 border-2 border-dashed border-indigo-500/20 rounded-xl flex flex-col items-center justify-center p-8 gap-2 text-center min-h-[120px]">
