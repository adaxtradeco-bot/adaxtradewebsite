...
export default function WhyNWMSection({
  ...
  return (
    <section className="py-12 md:py-20 px-4 md:px-6 bg-slate-50 dark:bg-slate-950 relative overflow-hidden">
      ...
      <div className="hidden md:block absolute top-1/4 right-0 w-96 h-96 bg-violet-500/10 rounded-full blur-3xl animate-pulse" />
      <div className="hidden md:block absolute bottom-1/4 left-0 w-96 h-96 bg-cyan-500/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-4 md:gap-6">
          {cards.map((card, idx) => (
            <article
              key={idx}
              className="relative overflow-hidden rounded-2xl md:rounded-3xl border border-slate-200 dark:border-slate-700/50 bg-gradient-to-br from-violet-500/20 to-white dark:from-violet-500/10 dark:to-slate-900/90 p-5 md:p-6 shadow-2xl md:backdrop-blur-sm hover:border-cyan-500/50 hover:shadow-cyan-500/20 md:hover:scale-[1.02] transition-all duration-500 group cursor-pointer"
            >
              {/* Decorative circle - animated on hover */}
              <div className="hidden md:block absolute -right-10 -bottom-10 w-28 h-28 rounded-full border border-dashed border-cyan-500/25 dark:border-cyan-500/25 group-hover:border-cyan-500/50 group-hover:scale-110 transition-all duration-500" />
              ...
              <h3 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white mb-2 md:mb-3 relative z-10 group-hover:text-cyan-400 dark:group-hover:text-cyan-400 transition-colors duration-300">{card.title}</h3>
              <p className="text-sm md:text-base text-slate-700 dark:text-slate-300 mb-3 md:mb-4 relative z-10">{card.description}</p>
