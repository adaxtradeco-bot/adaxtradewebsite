...
export default function AIAgentHeroSection({ data, style }: AIAgentHeroSectionProps) {
  ...
  useEffect(() => {
    if (data.enableParticles && particlesRef.current) {
      for (let i = 0; i < 25; i++) {
        ...
        particle.className = 'absolute w-0.5 h-0.5 bg-indigo-500/60 rounded-full animate-float-particle';
  ...
  return (
    <section
      className={`relative min-h-screen flex flex-col items-center justify-center text-center px-8 pt-32 pb-16 overflow-hidden ${style?.backgroundColor || 'bg-slate-950'} ${style?.textColor || 'text-white'} ${style?.padding || ''}`}
    >
      {/* Background Effects */}
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        {data.eyebrow && (
          <div className="inline-flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/25 text-indigo-300 text-xs font-semibold tracking-wider uppercase px-4 py-1.5 rounded-full mb-8">
        ...
        <p className="text-lg text-slate-300 max-w-2xl mx-auto leading-relaxed mb-12">
          {data.subtitle || 'Your organization doesn\'t need more tools — it needs intelligence built into every step.'}
        </p>
        ...
        <div className="flex gap-3 justify-center flex-wrap mb-16">
          <button className="flex items-center gap-2 bg-gradient-to-r from-indigo-500 via-violet-500 to-cyan-500 text-white px-8 py-4 rounded-xl text-base font-bold shadow-[0_0_50px_rgba(99,102,241,0.4)] hover:shadow-[0_0_80px_rgba(99,102,241,0.6)] hover:-translate-y-0.5 transition-all">
            {data.primaryButtonIcon && <span dangerouslySetInnerHTML={{ __html: data.primaryButtonIcon }} />}
            {data.primaryButtonText || 'Get Started Free'}
          </button>
          <button className="flex items-center gap-2 bg-white/5 text-white border border-white/20 px-8 py-4 rounded-xl text-base font-medium hover:bg-white/10 transition-all">
            {data.secondaryButtonIcon && <span dangerouslySetInnerHTML={{ __html: data.secondaryButtonIcon }} />}
            {data.secondaryButtonText || 'Watch a Demo'}
          </button>
        </div>

        {data.stats && data.stats.length > 0 && (
          <div className="flex gap-0.5 bg-white/5 border border-white/10 rounded-2xl overflow-hidden max-w-3xl mx-auto">
            {data.stats.map((stat, idx) => (
              <div key={idx} className="flex-1 bg-slate-900/50 backdrop-blur-sm p-5 text-center">
      ...
      <style jsx>{`
        ...
        .animate-drift-delayed { animation: drift 10s ease-in-out infinite alternate -4s; }
        .animate-drift-slow { animation: drift 10s ease-in-out infinite alternate -7s; }
